...
test("installer merges MCP registration and Pi extension without clobbering existing settings", () => {
  ...
  try {
    ...
    expect(settings.mcpServers.existing.command).toBe("other");
    expect(settings.mcpServers.circadian).toEqual({ command: process.execPath, args: [path.join(install, "src/serve.ts")], env: { CIRCADIAN_HOME: install } });
