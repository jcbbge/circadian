...
---
## YOUR RED TESTS — in `src/stack.test.ts` (do not edit that file)

Run: `bun test src/stack.test.ts`

1. `poison quarantine — corpus containment > a read-episode failure (missing
   file) does not block a later episode in the same corpus`
2. `poison quarantine — corpus containment > a parse-episode failure (no
   frontmatter date) does not block a later episode in the same corpus`

Both fail today at `expect(second.length).toBeGreaterThan(0)` (Received: 0) —
the second corpus file is never attempted because `fail()` exits the process.

3. **THE LIVE-GATED ONE — this is where your grant is spent:**
   `poison quarantine — the actual poison episode (LIVE-LLM gated, fact 11)`,
   skipped by default. Run it **once**, deliberately:
   `CIRCADIAN_LIVE_LLM=1 bun test src/stack.test.ts -t "actual poison episode"`
   Take `ps -p 1054 -o pid,pcpu,pmem` **before and after** and report both. If it
   exceeds 60% sustained, STOP, report, and continue with everything else.
   This same run is your diagnosis vehicle — capture WHY the episode fails.

Read `briefs/rem-storm/criteria-B.md` for the full mapping.
