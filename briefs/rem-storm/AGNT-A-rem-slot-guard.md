...
---
## YOUR RED TESTS — all 4 in `src/rem-popmem.test.ts` (do not edit that file)

Run: `bun test src/rem-popmem.test.ts`

1. `single-flight lock — whole entry path (work item 2) > a live --if-due lock
   also blocks a concurrent MANUAL (non---if-due) run`
2. `consecutive-failure budget & slot-burn (work items 1 and 3) > a failed pass
   burns its slot: isDue must not re-offer within the same slot after a failure`
3. `consecutive-failure budget & slot-burn (work items 1 and 3) > three
   consecutive failed passes across three slots stop the fourth from starting;
   a clean manual run clears the stuck state`
4. `episode-level failure no longer wedges the pass (work items 4 and 5) > one
   malformed episode is held aside; the remaining episodes still process and
   successes are recorded digested`

**Fixture contract the tests already encode — code to it, do not redesign it:**
failed-slot scoreboard entries carry `{ts, type:"rem", failed:true,
failure_episode}`, and the degraded event's `context.failure_episode` names the
blocker. Read `briefs/rem-storm/criteria-A.md` for the full mapping.

**LIVE-LLM GRANT: NO.** The tests point at closed port 19999 by design.
